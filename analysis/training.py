...
def create_random_path(track_line, track_width, weight = 0.3):
    ...
    for i in range(1, len(track_line) - 1):
...
def mutate(centerline, path, mutation_rate, track_width, temperature):
    ...
    while i < len(path)-2:
        if np.random.rand() < mutation_rate:
            ...
            quant = int(np.round(np.random.uniform(low = 1, high = MUTATION_SHIFT)))
...
def genetic_algorithm(kart, centerline, track_line, track_width, track_graph, track_img):
    ...
    # Main loop
    x = 0
    while temperature > 0.01:  # Stopping criterion (you can adjust this)
        new_population = []
        x+=1
        start = time.time()
        ...
        track_with_path = add_line(track_img.copy(), best_path)
...
def train(track_line):
    centerline = np.array(pkl.load(open(utils.get_base_file_path('saves/centerline.pkl'), 'rb')))
...
improved_line = np.array(pkl.load(open(utils.get_base_file_path('saves/best_path_1.pkl'), 'rb')))
train(improved_line)